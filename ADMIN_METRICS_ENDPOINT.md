...
- **aiStats.image** – по кожному AI‑сервісу для image‑генерації: скільки нових постів було створено за **останні 7 днів** (`newPosts`).  
  **Примітки**: 
  - Пости без вказаного `ai_service` в `generation_params` (legacy пости) автоматично зараховуються до `"flux"`.
  - Враховуються тільки валідні image‑AI‑сервіси з бази (`AISettingsEntity` де `type='image'`). Якщо в `generation_params` стоїть video‑сервіс (наприклад, `byty_dance`), він ігнорується в image stats.
- **aiStats.video** – по кожному AI‑сервісу для video‑генерації: скільки нових відео‑постів було створено за **останні 7 днів** (`newPosts`).  
  **Примітки**: 
  - Відео‑пости без вказаного `ai_service` автоматично зараховуються до `"byty_dance"`.
  - Враховуються тільки валідні video‑AI‑сервіси з бази (`AISettingsEntity` де `type='video'`). Якщо в `generation_params` стоїть image‑сервіс (наприклад, `flux`), він автоматично перекласифікується на `"byty_dance"`.
