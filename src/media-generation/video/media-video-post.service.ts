...
interface CreateMediaVideoPostParams {
  requestId: string;
  providerJobId: string;
  providerModel: string;
  dto: GenerateMediaVideoDto;
...
@Injectable()
export class MediaVideoPostService {
  constructor(
  ...
  async createGeneratedPost(params: CreateMediaVideoPostParams): Promise<{
    post: PostEntity;
    payloadItem: {
      id: number;
      videoUrl: string;
    ...
    const fallbackTag = params.primaryTag ?? (await this.getDefaultTag());
    const suggestedTags = this.buildSuggestedTags(fallbackTag);
    ...
    const post = await this.postRepository.save(
      this.postRepository.create({
        ...
        tag: fallbackTag ? ({ id: fallbackTag.id } as any) : null,
        ...(params.dto.contestId ? { contest: { id: params.dto.contestId } } : {}),
        videoUrl: params.uploadedVideoUrl,
        imageUrl: null,
        hasAudio: false,
        previewImageUrl:
          params.request.imageUrl ||
          this.generateCloudinaryPreviewUrl(params.uploadedVideoUrl),
        isPublished: false,
        isSaved: true,
        generationParams,
      }),
  ...
  private buildGenerationParams(params: CreateMediaVideoPostParams & {
    suggestedTags: { id: number; name: string; imageUrl: string }[];
    ...
    return {
      ...
      source_image_url: params.request.imageUrl,
