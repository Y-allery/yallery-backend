export const USER_SWAGGER = {
  generateReferralCode: {
    ...
    description: `Generate a referral code for the authenticated user. Idempotent: a user who already has a code always gets that same code back, so the link stays stable wherever it was shared.

The code is reusable — share it with as many people as you like. Each of them may redeem it once.`,
    responses: {
      success: { status: 200, description: 'Referral code returned' },
    },
  },
  useReferralCode: {
    summary: 'Use referral code',
    description: `Apply another user's referral code. A code is a durable invite and may be redeemed by many people; what is single-use is the caller's own bonus.

Refusals carry a machine-readable \`code\` next to the human \`message\` — branch on that, not on the text:
- \`referral_code_not_found\` (404)
- \`own_referral_code\` (400) — your own code, or a second account on the same mailbox
- \`referral_bonus_already_used\` (400) — you have already redeemed a code`,
    responses: {
      success: {
        status: 200,
      ...
      badRequest: {
        status: 400,
        description:
          'Own code, or the bonus was already used. See the `code` field.',
      },
    },
  },
  bindPartner: {
...
- \`puid_already_bound\` (409) — the puid belongs to another account and is never overwritten`,
  ...
  unregisterDeviceTokensByType: {
    ...
    responses: {
      success: {
        status: 200,
        description: 'Device tokens unregistered successfully',
      },
      notFound: {
        status: 404,
        description: 'No tokens found for this device type',
      },
    },
  },
  ...
  updateAvatar: {
    ...
    responses: {
      ...
      badRequest: {
        status: 400,
        description: 'Invalid file or file is required',
      },
    },
  },
  changePassword: {
  ...
  updateNickname: {
    ...
    responses: {
      ...
      badRequest: {
        status: 400,
        description: 'Nickname already taken or invalid',
      },
    },
  },
  updateTwitterUsername: {
    ...
    responses: {
      success: {
        status: 200,
        description: 'Twitter username updated successfully',
      },
    },
  },
  logReferralActivity: {
